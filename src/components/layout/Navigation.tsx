...
export default function Navigation() {
  ...
  const { isMenuOpen, setIsMenuOpen, setCursorVariant, currentSection } = useAppStore()
  ...
  return (
    <>
      ...
      >
        <nav className={`mx-auto max-w-7xl px-6 ${scrolled ? 'glass rounded-2xl mx-6' : ''}`}>
          <div className="flex items-center justify-between h-16">
              ...
              {navItems.map((item) => (
                ...
                >
                  ...
                  {currentSection === item.id && (
                    <motion.span
                      layoutId="activeNav"
                      className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-accent-purple to-accent-pink"
            ...
            <div className="hidden lg:flex items-center gap-4">
              <motion.a
                ...
                className="px-5 py-2.5 rounded-xl font-medium glass hover:bg-white/10 transition-all duration-300"
                onMouseEnter={() => setCursorVariant('hover')}
                onMouseLeave={() => setCursorVariant('default')}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              ...
              <motion.a
                href="#contact"
                className="px-5 py-2.5 rounded-xl font-medium bg-gradient-to-r from-accent-purple to-accent-pink hover:opacity-90 transition-opacity"
                onMouseEnter={() => setCursorVariant('hover')}
                onMouseLeave={() => setCursorVariant('default')}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
            ...
            <div className="relative h-full flex flex-col items-center justify-center gap-8 p-8">
              {navItems.map((item, i) => (
                <motion.a
                  ...
                  initial="closed"
                  animate="open"
                  className="text-4xl md:text-5xl font-display font-bold text-white hover:text-gradient transition-all duration-300"
